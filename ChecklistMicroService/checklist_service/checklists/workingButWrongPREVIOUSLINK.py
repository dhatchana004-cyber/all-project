...
class RoleBasedChecklistTRANSFERRULEAPIView(APIView):
    ...
    def get_user_accesses(self, request, user_id, project_id, headers):
        """
        Pulls user-access (category branches). On failure, returns [] (no filter).
        """
        try:
            resp = requests.get(
    ...
    def build_category_q(self, request, accesses):
        """
        If query params provide category/category_level*, prefer that single branch.
        Else build OR-union of branches from user-access list.
        If nothing found, returns Q() (i.e., no category narrowing).
        """
        keys = ["category"] + [f"category_level{i}" for i in range(1, 7)]
        has_any = any(k in request.query_params for k in keys)
        if has_any:
            q = Q()
            try:
                if "category" in request.query_params:
                    q &= Q(category=int(request.query_params.get("category")))
                for i in range(1, 7):
                    k = f"category_level{i}"
                    if k in request.query_params:
                        q &= Q(**{k: int(request.query_params.get(k))})
            except Exception:
                return Q()
            self.dbg("build_category_q_from_params", has_any=True)
            return q
        ...
        for acc in accesses or []:
            if acc.get('category') is None:
                continue
            union_q |= self.get_branch_q(acc)
            added_any = True
        self.dbg("build_category_q_from_access", added_any=added_any)
        return union_q if added_any else Q()
    ...
    # ---------------------------
    # Paginators with group_complete
    # ---------------------------
    ...
    def paginate_and_group_checker(self, request, assigned_checklists, available_checklists, headers,
        ...
        # We also need stage_id/purpose_id for group_complete.
        for checklist in paginated_checklists:
    ...
    # ---------- Generic room-wise for initializer ----------
...
#         If nothing found, returns Q() (i.e., no category narrowing).
#         """
#         # Try query params first (optional future-proofing)
#         keys = ["category"] + [f"category_level{i}" for i in range(1, 7)]
#         has_any = any(k in request.query_params for k in keys)
#         if has_any:
#             q = Q()
...
#                         q &= Q(**{k: int(request.query_params.get(k))})
#             except Exception:
#                 # fallback to no-op if params malformed
#                 return Q()
#             return q

#         # Else: user-access union
#         union_q = Q()
#         added_any = False
#         for acc in accesses or []:
#             if acc.get('category') is None:
...
#         return Checklist.objects.filter(checklist_filter)
...
#         # If we are at checklist_level, just ensure category filter (if any) and all-items status
#         if true_level == "checklist_level":
#             base = checklists.filter(category_q) if category_q else checklists
#             filtered = self.all_items_have_status(base, allowed_statuses)
#             return base.filter(id__in=[c.id for c in filtered])

#         # Else group by location
#         if true_level in ["flat_level", "room_level", "zone_level"]:
#             location_field = {
#                 "flat_level": "flat_id",
#                 "room_level": "room_id",
...
#                 if len(self.all_items_have_status(group_checklists, allowed_statuses)) == group_checklists.count() and group_checklists.count() > 0:
#                     qs = qs.union(group_checklists)
...
#         # Category filter intentionally NOT applied for initializer
#         filtered_qs = base_qs.filter(checklist_filter)

#         # If category_filter was required for initializer, we'd OR union here — but spec says no.
#         status_param = request.query_params.get("status")
#         if status_param:
#             if "," in status_param:
#                 statuses = [s.strip() for s in status_param.split(",")]
...
#         # Category Q (from user-access union; UI not sending)
#         accesses = self.get_user_accesses(request, user_id, project_id, headers)
#         category_q = self.build_category_q(request, accesses)
#         base_qs = base_qs.filter(category_q) if category_q else base_qs
...
#         # If project set to skip supervisor, return empty structure
#         if self.get_project_skip_supervisory(project_id, headers=headers):
#             return self.paginate_and_group_supervisor(
#                 request, [], [], headers,
#                 true_level=self.get_true_level(project_id),
#                 allowed_statuses=self.ROLE_STATUS_MAP["supervisor"],
#                 category_q=Q()  # no effect
#             )
...
#     # ---------- Manager/Client (read-only, keep simple; no category unless you want) ----------
...
#         # We also need stage_id/purpose_id for group_complete.
#         # All checklists are of the current stage; still, read from the first checklist per room.
#         for checklist in paginated_checklists:
#             room_id = checklist.room_id
#             if room_id and room_id in room_details:
#                 grouped[room_id]["room_details"] = room_details[room_id]
...
#         # compute group_complete per room
#         for room_id, payload in grouped.items():
#             # pick any checklist in this room to read purpose/stage
#             pick = None
#             src = payload["assigned_to_me"] or payload["available_for_me"]
#             if src:
#                 # src holds serialized dicts; get ids to fetch real object
#                 checklist_ids = [c["id"] for c in src]
#                 any_c = Checklist.objects.filter(id__in=checklist_ids).first()
#                 if any_c:
#                     purpose_id = any_c.purpose_id
...
#         # user-generated (roomless) — apply category filter too
#         user_generated_qs = Checklist.objects.filter(
#             user_generated_id__isnull=False,
#             project_id=project_id,
#             room_id__isnull=True
...
#         # user-generated — apply category filter too (supervisor typically won’t see these pending_for_supervisor)
#         user_generated_qs = Checklist.objects.filter(
#             user_generated_id__isnull=False,
#             project_id=project_id,
#             room_id__isnull=True
...
#         # user-generated — apply category filter too
#         user_generated_qs = Checklist.objects.filter(
#             user_generated_id__isnull=False,
#             project_id=project_id,
#             room_id__isnull=True
...
class VerifyChecklistItemForCheckerNSupervisorAPIView(APIView):
    ...
    def _clone_group_to_next_stage(*, source_group_qs, next_phase_id, next_stage_id):
        ...
        # 2) clone items (⚠️ your model has no `sequence` field)
        old_to_new_item = {}
        new_items = []
...
#     def paginate_and_group_maker(self, request, assigned_checklists, available_checklists, headers):
#         """
#         Grouping and pagination for maker response (room-wise), just like checker/supervisor.
#         Only difference: items filtered by status="pending_for_maker"
#         """
#         all_checklists = list(assigned_checklists) + list(available_checklists)
#         seen = set()
#         unique_checklists = []
...
#             checklist_data = ChecklistSerializer(checklist).data
#             # Only items for maker status
#             items = ChecklistItem.objects.filter(checklist=checklist.id, status="pending_for_maker")
#             checklist_data['items'] = []
#             for item in items:
#                 item_data = ChecklistItemSerializer(item).data
#                 options = ChecklistItemOption.objects.filter(checklist_item=item.id)
...
#         if true_level == "checklist_level":
#             filtered = self.all_items_have_status(checklists, allowed_statuses)
#             return checklists.filter(id__in=[c.id for c in filtered])
...
#                 if len(self.all_items_have_status(group_checklists, allowed_statuses)) == group_checklists.count():
#                     qs = qs.union(group_checklists)
...
#     def get_or_create_stage_history(self, project_id, phases, stages, zone_id=None, flat_id=None, room_id=None, current_purpose_id=None):
#         try:
#             filters = {"project": project_id, "is_current": True}
#             if flat_id is not None:
#                 filters["flat"] = flat_id
...
#         headers = {"Authorization": f"Bearer {token}"} if token else {}
#         if self.get_project_skip_supervisory(project_id, headers=headers):
#             return self.paginate_and_group_supervisor(request, [], [], headers)
...
#     def handle_maker(self, request, user_id, project_id):
#         """
#         Returns grouped room-wise checklists for the Maker role.
#         Each group contains 'assigned_to_me' and 'available_for_me' arrays,
#         with full checklist dicts, each having filtered items for pending_for_maker,
#         and each item has options and submissions.
#         """
#         true_level = self.get_true_level(project_id)
#         token = None
#         auth_header = request.headers.get("Authorization")
...
#         skip_super = self.get_project_skip_supervisory(project_id, headers=headers)
#         if skip_super:
#             allowed_statuses = ["pending_for_maker", "tetmpory_inspctor", "completed"]  
#         else:
...
#         # Pagination (do this for the union of both, just like checker/supervisor)
#         all_checklists = list(assigned_to_me) + list(available_for_me)
#         seen = set()
#         unique_checklists = []
#         for c in all_checklists:
#             if c.id not in seen:
#                 unique_checklists.append(c)
#                 seen.add(c.id)
#         paginator = LimitOffsetPagination()
#         paginator.default_limit = 10
#         paginated_checklists = paginator.paginate_queryset(unique_checklists, request, view=self)
...
#         # Group by room, then split assigned/available
#         grouped = defaultdict(lambda: {
#             "room_id": None,
#             "room_details": None,
#             "assigned_to_me": [],
...
#             checklist_data = ChecklistSerializer(checklist).data
#             # Only include items that are pending_for_maker
#             items = ChecklistItem.objects.filter(checklist=checklist, status="pending_for_maker")
#             checklist_data['items'] = []
#             for item in items:
#                 item_data = ChecklistItemSerializer(item).data
#                 # Attach options and submissions for each item
#                 options = ChecklistItemOption.objects.filter(checklist_item=item.id)
#                 item_data['options'] = ChecklistItemOptionSerializer(options, many=True).data
#                 submissions = ChecklistItemSubmission.objects.filter(checklist_item=item.id)
#                 item_data['submissions'] = ChecklistItemSubmissionSerializer(
...
#             if checklist in assigned_to_me:
#                 grouped[room_id]["assigned_to_me"].append(checklist_data)
#             if checklist in available_for_me:
#                 grouped[room_id]["available_for_me"].append(checklist_data)

#         # Prepare response_data in the same format as checker/supervisor
#         response_data = [
#             room_data for room_data in grouped.values()
#             if room_data["assigned_to_me"] or room_data["available_for_me"]
#         ]

#         # User-generated checklists section
#         user_generated_qs = Checklist.objects.filter(
#             user_generated_id__isnull=False,
#             project_id=project_id,
#             room_id__isnull=True
