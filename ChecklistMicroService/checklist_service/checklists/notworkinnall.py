...
class RoleBasedChecklistTRANSFERRULEAPIView(APIView):
    ...
    def fetch_stage_history(self, project_id, headers, key, loc_id, is_current=None, ordering=None):
        """
        Calls StageHistoryListView endpoint.
        """
        try:
            ...
            resp = requests.get(self.STAGE_HISTORY_API, params=params, headers=headers, timeout=5)
            if resp.status_code == 200:
                return resp.json() or []
        except Exception as e:
    ...
    def stage_gate_precheck(self, request, project_id, headers, true_level, accesses):
        ...
        if assigned_stage_id is None:
            # Fail-closed per your instruction
        ...
        if not key or not loc_id:
            # No concrete location to gate on -> allow
        ...
        if not current_rows:
            history = self.fetch_stage_history(project_id, headers, key, loc_id, is_current=None, ordering="stage,started_at")
            note = f"No current stage found for location; your access stage is {assigned_stage_id}."
            self.dbg("stage_gate", reason="no_current_row -> block")
        ...
        history = self.fetch_stage_history(project_id, headers, key, loc_id, is_current=None, ordering="stage,started_at")
        note = f"Current stage is {current_stage}; your access is for stage {assigned_stage_id}."
...
# ---------- Yellow debug printer ----------
def dbg(tag, payload=None):
    Y = "\033[93m"  # yellow
    R = "\033[0m"   # reset
    if payload is not None:
        try:
            print(f"{Y}[DBG] {tag}: {pformat(payload)}{R}")
        except Exception:
            print(f"{Y}[DBG] {tag}: {payload}{R}")
    else:
        print(f"{Y}[DBG] {tag}{R}")
# ------------------------------------------
...
class UserGeneratedChecklist(APIView):
    ...
    # Avoid trailing slash to prevent double slashes in URLs
    ...
    def get_user_role(self, request, user_id, project_id):
        ...
        dbg("Role API Request", {"url": url, "headers_present": bool(auth_header)})

        try:
            resp = requests.get(url, headers=headers, timeout=10)
            dbg("Role API Response", {
                "status_code": resp.status_code,
                "text": resp.text[:500]  # cap to 500 chars
            })
            if resp.status_code == 200:
                return resp.json().get("role")
        except Exception as e:
            dbg("Role API Exception", str(e))
        return None
    ...
    def post(self, request):
        ...
        dbg("Incoming request.user", {"id": user.id, "username": getattr(user, "username", None)})
        # NOTE: Don’t print whole headers (tokens), just meta
        dbg("Incoming headers meta", {
            "has_authorization": bool(request.headers.get("Authorization")),
            "content_type": request.headers.get("Content-Type")
        })
        dbg("Incoming data", data)

        # Required fields (make phase_id & stage_id compulsory now)
        required_fields = [
            "flat_id", "project_id", "name", "purpose_id", "phase_id", "stage_id"
        ]
        missing_fields = [field for field in required_fields if data.get(field) in (None, "", [])]
        if missing_fields:
            dbg("Missing required fields", missing_fields)
            return Response(
        ...
        dbg("Resolved role", role)
        if not role or role.upper() not in self.ALLOWED_ROLES:
            dbg("Role not allowed", {"role": role, "allowed": self.ALLOWED_ROLES})
            return Response(
                {"detail": f"Only users with roles: {', '.join(self.ALLOWED_ROLES)} can create this checklist."},
                status=status.HTTP_403_FORBIDDEN
            )

        # Build fields safely; pass-through optional location/category fields if present
        checklist_fields = {
            "name": data["name"],
        ...
        dbg("Checklist fields (pre-create)", checklist_fields)

        # Validate items
        items = data.get("items")
        if not items or not isinstance(items, list):
            dbg("Items validation failed", items)
            return Response({"error": "At least one item (list) is required."}, status=status.HTTP_400_BAD_REQUEST)

        for idx, item in enumerate(items):
            if not item.get("title"):
                dbg("Item missing title", {"index": idx, "item": item})
                return Response({"error": f"Item at index {idx} missing 'title'."}, status=status.HTTP_400_BAD_REQUEST)
            options = item.get("options")
            if not options or not isinstance(options, list):
                dbg("Item missing options", {"index": idx, "item": item})
                return Response({"error": f"Item '{item.get('title', idx)}' must have at least one option."},
                                status=status.HTTP_400_BAD_REQUEST)

        dbg("Items validated OK", {"count": len(items)})

        try:
            with transaction.atomic():
                checklist = Checklist.objects.create(**checklist_fields)
                dbg("Checklist created", {"id": checklist.id, "name": checklist.name})
                ...
                for i, item in enumerate(items):
                    ...
                    dbg("ChecklistItem created", {"idx": i, "id": checklist_item.id, "title": checklist_item.title})

                    for j, option in enumerate(item["options"]):
                        if not option.get("name"):
                            dbg("Option missing name", {"item_idx": i, "option_idx": j, "option": option})
                            return Response(
                                {"error": f"Checklist item '{item.get('title')}' option missing 'name'."},
                                status=status.HTTP_400_BAD_REQUEST
                            )
                        opt = ChecklistItemOption.objects.create(
                            checklist_item=checklist_item,
                            name=option.get("name"),
                            choice=option.get("choice", "P"),
                        )
                        dbg("ChecklistItemOption created", {
                            "item_idx": i, "option_idx": j, "id": opt.id, "name": opt.name, "choice": opt.choice
                        })

                # Transition checklist status
                checklist.status = "work_in_progress"
                checklist.save(update_fields=["status"])
                dbg("Checklist status updated", {"id": checklist.id, "status": checklist.status})

                # Submissions (one rejected_by_checker + one created per item)
                inspector_photo = request.FILES.get("inspector_photo")
                dbg("Inspector photo present", bool(inspector_photo))

                for ci in checklist.items.all():
                    s1 = ChecklistItemSubmission.objects.create(
                        checklist_item=ci,
                        status="rejected_by_checker",
                        checker_id=user.id,
                        checker_remarks=data.get("checker_remarks", ""),
                        checked_at=timezone.now(),
                        inspector_photo=inspector_photo,
                        attempts=1,
                    )
                    dbg("Submission created", {
                        "type": "rejected_by_checker",
                        "submission_id": s1.id,
                        "checklist_item_id": ci.id
                    })

                    s2 = ChecklistItemSubmission.objects.create(
                        checklist_item=ci,
                        status="created",
                        checker_id=user.id,
                        attempts=2,
            ...
            dbg("Exception during checklist create", str(e))
            return Response(
                {"error": f"Failed to create checklist: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )

        dbg("Checklist creation success", {"checklist_id": checklist.id})
        return Response(
