...
class MAker_DOne_view(APIView):
    ...
    def post(self, request):
        ...
        _maker_files = _files_from_json_array(request.data.get("maker_media_multi"), filename_prefix="maker")
        ...
        if _maker_files:
            bulk_rows = [
                ChecklistItemSubmissionImage(
                    ...
                    remarks=(maker_remark or None),
                )
                for f in _maker_files if f
            ]
            if bulk_rows:
                ChecklistItemSubmissionImage.objects.bulk_create(bulk_rows)
                        ...
                        if data.get("workflow_completed") is True:
                            ...
                            if repo_on:
                                try:
                                    ...
                                    # do not fail the stage advance if clone fails
                                    if advancement_info is None:
                                        advancement_info = {}
                                    if isinstance(advancement_info, dict):
    ...
    def patch(self, request):
        ...
        if role.lower() == "checker":
            ...
            if option.choice == "P":
                ...
                _chk_files = _files_from_json_array(request.data.get("checker_media_multi"), filename_prefix="checker")
                ...
                if _chk_files:
                    bulk_rows = [
                        ChecklistItemSubmissionImage(
                            ...
                            who_did="checker",
                            uploaded_by_id=getattr(request.user, "id", None),
                            remarks=(check_remark or None),
                        )
                        for f in _chk_files if f
                    ]
                    if bulk_rows:
                        ChecklistItemSubmissionImage.objects.bulk_create(bulk_rows)
                ...
                _chk_files = []
                _chk_files += request.FILES.getlist("checker_media_multi") or []
                _chk_files += request.FILES.getlist("checker_media_multi[]") or []
                _chk_files += request.FILES.getlist("inspector_photos[]") or []
                _chk_files += request.FILES.getlist("files[]") or []
            ...
            if option.choice == "P":
                ...
                _sup_files = _files_from_json_array(request.data.get("supervisor_media_multi"), filename_prefix="supervisor")
                ...
                if _sup_files:
                    bulk_rows = [
                        ChecklistItemSubmissionImage(
                            ...
                            who_did="supervisor",
                            uploaded_by_id=getattr(request.user, "id", None),
                            remarks=(supervisor_remark or None),
                        )
                        for f in _sup_files if f
                    ]
                    if bulk_rows:
                        ChecklistItemSubmissionImage.objects.bulk_create(bulk_rows)
                ...
                _sup_files = []
                _sup_files += request.FILES.getlist("supervisor_media_multi") or []
                _sup_files += request.FILES.getlist("supervisor_media_multi[]") or []
                _sup_files += request.FILES.getlist("reviewer_photos[]") or []
                _sup_files += request.FILES.getlist("files[]") or []
