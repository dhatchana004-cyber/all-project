...
class MAker_DOne_view(APIView):
    ...
    def post(self, request):
        ...
        # 3) For skip_super flows, flip slice → category-aware
        if skip_super:
            ...
            # Similarly, only re-open temp maker slice when all others are in allowed temp states
        ...
        # 4) Maker_to_checker advancement (category-aware)
        ...
        if skip_super and maker_to_checker:
            try:
                ...
                branch_q_full = self._category_branch_q_from_checklist(checklist)

                # Restrict to same project+stage+location **and** same category branch
                group_checklists = Checklist.objects.filter(**group_fk_full).filter(branch_q_full)

                # condition: no items left in pending_for_maker in THIS category slice
                ...
                if not maker_open_exists:
                    ...
                    if not current_sh:
                        ...
                        if data.get("workflow_completed") is True:
                            ...
                            # clone repository for THIS category slice only
                            if repo_on:
                    ...
                    advancement_info = "Maker group not complete yet (category slice)"
            except Exception as e:
...
class RoleBasedChecklistTRANSFERRULEAPIView(APIView):
    ...
    def fetch_stage_history(self, project_id, headers=None, key=None, loc_id=None,
        ...
        """
        Query StageHistory directly from the local DB instead of hitting remote API.
        """
        try:
    ...
    #         if key and loc_id:
    #             # Map incoming key (flat_id / room_id / zone_id) to ORM filter
    #             filters[key] = loc_id
    #         if is_current is not None:
    #             filters["is_current"] = bool(is_current)
    ...
    #         # Convert QuerySet → list[dict] for same JSON-like response
    #         result = [
    #             {
    #                 "id": sh.id,
    #                 "project": sh.project_id,
    #                 "stage": sh.stage_id,
    #                 "phase_id": sh.phase_id,
    #                 "flat_id": sh.flat_id,
    #                 "zone_id": sh.zone_id,
    ...
    #                 "started_at": sh.started_at,
    #                 "completed_at": sh.completed_at,
    #                 "completed_by": sh.completed_by_id,
    #             }
    #             for sh in qs
    #         ]
