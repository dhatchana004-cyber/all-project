...
def get_project_flags(project_id, headers=None):
    """
    Return dict like {"skip_supervisory": bool}. Defaults to False on errors.
    """
    try:
        ...
        if resp.status_code == 200:
            data = resp.json()
            return {"skip_supervisory": bool(data.get("skip_supervisory", False))}
    except Exception:
        pass
    return {"skip_supervisory": False}
