...
class RoleBasedChecklistTRANSFERRULEAPIView(APIView):
    ...
    def _compute_stage_note_and_history(self, request, project_id, headers, true_level=None):
        ...
        try:
            ...
            if status == "completed":
                ...
                if crm_hoto or crm_date or crm_name or crm_by:
                    who = crm_name or (f"User {crm_by}" if crm_by else None)
                    if who and crm_date:
                        return (f"CRM done by {who} on {crm_date}", rows_out)
                    elif who:
                        return (f"CRM done by {who}", rows_out)
                    else:
                        return ("CRM done", rows_out)

                # no CRM yet
                return ("Completed CRM PENDING", rows_out)
    ...
    def _attach_crm_meta(self, response, request, note, sh_rows):
        """
        Adds two additive fields to response:
        - crm_by_name:    who completed the stage (for "by {name}" in UI)
        - crm_stagehistory_id: only set for the finisher while CRM is empty
        """
        crm_by_name = None
        crm_stagehistory_id = None

        if sh_rows:
            sh = sh_rows[0]
            crm_by_name = (
                sh.get("completed_by_name")
                or (f"User {sh.get('completed_by')}" if sh.get("completed_by") else None)
            )
            if note == "Completed CRM PENDING" and self._crm_gate(request, sh):
                crm_stagehistory_id = sh.get("id")

        # Non-breaking additions
        response.data["crm_by_name"] = crm_by_name
        response.data["crm_stagehistory_id"] = crm_stagehistory_id
        return response
    ...
    def stage_gate_precheck(self, request, project_id, headers, true_level, accesses):
        ...
        if not current_rows:
            ...
            note = f"No current stage found for this location. Your access stage is {assigned_label}."
