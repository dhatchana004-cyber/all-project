...
class StageHistoryCRMUpdateAPIView(APIView):
    ...
    def post(self, request):
        ...
        # 3) basic permission: only finisher or staff
        is_finisher = (sh.completed_by == request.user.id)
        if not is_finisher and not request.user.is_staff:
        ...
        fields_to_update.append("crm_hoto")

        # crm_date: if missing/null, set to today
        crm_date = data.get("crm_date")
        if not crm_date:
            crm_date = timezone.now().date()
        sh.crm_date = crm_date
        fields_to_update.append("crm_date")

        # 5) auto-fill who did the CRM tagging
        sh.crm_completed_by = request.user.id
        sh.crm_completed_by_name = (
            getattr(request.user, "get_full_name", lambda: "")() or
            getattr(request.user, "username", None) or
            str(request.user.id)
        )
        fields_to_update += ["crm_completed_by", "crm_completed_by_name"]

        sh.save(update_fields=list(set(fields_to_update)))
