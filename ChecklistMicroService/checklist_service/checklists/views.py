...
from .models import ChecklistItemSubmissionImage
import base64, json, mimetypes, re, uuid
from django.core.files.base import ContentFile

DATAURL_RE = re.compile(r'^data:(?P<mime>[\w/+.-]+);base64,(?P<data>.+)$')
...
def _files_from_json_array(value, *, filename_prefix="upload"):
    """
    Accepts:
      - value: list OR JSON-string of:
          * base64 strings
          * data URLs ("data:image/png;base64,...")
          * dicts: {"name": "...", "data": "<base64 or data-url>"}
    Returns list[ContentFile].
    """
    files = []
    if value is None:
        return files
    ...
    if isinstance(value, str):
        try:
            ...
            # Not JSON -> treat as single base64/data-url string
            value = [value]

    if not isinstance(value, (list, tuple)):
        return files

    for idx, raw in enumerate(_flatten_list(value)):
        if not raw:
            continue

        name = f"{filename_prefix}_{uuid.uuid4().hex[:8]}"
        data = None

        if isinstance(raw, dict):
            data = raw.get("data") or raw.get("base64")
        ...
        # data URL?
        m = DATAURL_RE.match(data or "")
        if m:
            b = base64.b64decode(m.group("data"))
            ext = mimetypes.guess_extension(m.group("mime")) or ".bin"
            files.append(ContentFile(b, name=name + ext))
            continue
        ...
        try:
            ...
            continue
        files.append(ContentFile(b, name=name))
...
class MAker_DOne_view(APIView):
    ...
    def post(self, request):
        ...
        print("🟨 MAKER | multi upload summary:",
                ...
                if not maker_open_exists:
                    ...
                    if not current_sh:
                        ...
                        if data.get("workflow_completed") is True:
                            ...
                            if repo_on:
                                try:
                                    source_group_qs = group_checklists  # TRUE-LEVEL scope across ALL categories
                                    if source_group_qs.exists():
                                        ...
                                        # annotate advancement_info for debugging/observability
...
class VerifyChecklistItemForCheckerNSupervisorAPIView(APIView):
    ...
    def _collect_multi_files(self, request, base_keys=("checker_media_multi",), include_single=None):
        files = []
        variants = []
        for k in base_keys:
            variants.extend([k, f"{k}[]"])
    ...
    def _clone_group_to_next_stage(*, source_group_qs, next_phase_id, next_stage_id):
        ...
        from checklists.models import (
            Checklist, ChecklistItem, ChecklistItemOption,
            ChecklistItemSubmission, ChecklistItemSubmissionImage
        )
        ...
        #    We must build a reliable mapping: old_submission_id -> new_submission
        ...
        for old_item_id, new_item in old_to_new_item.items():
            ...
            for s in subs:
                ...
                # Create individually to obtain a PK now
                new_s.save()
        ...
        if old_to_new_submission:
            ...
            imgs = ChecklistItemSubmissionImage.objects.filter(submission_id__in=old_sub_ids)
            ...
            for img in imgs.iterator():
                # NOTE: we keep the same underlying file reference (no storage duplication)
                new_imgs.append(
    ...
    def patch(self, request):
        ...
        if role.lower() == "checker":
            ...
            if option.choice == "P":
                ...
                _chk_files = self._collect_multi_files(request, base_keys=("checker_media_multi",), include_single=None)
                print("🟨 CHECKER | multi upload summary:", len(_chk_files),
                ...
                print("🟨 CHECKER | multi upload summary (reject):", len(_chk_files),
                    [getattr(f, "name", None) for f in _chk_files])
            ...
            if option.choice == "P":
                ...
                _sup_files = self._collect_multi_files(request, base_keys=("supervisor_media_multi",), include_single=None)
                print("🟨 SUPERVISOR | multi upload summary:", len(_sup_files),
                ...
                print("🟨 SUPERVISOR | multi upload summary (reject):", len(_sup_files),
                    [getattr(f, "name", None) for f in _sup_files])
                if _sup_files:
                    saved = self._save_submission_images(
                        submission=submission,
...
class StageHistorySummaryAPIView(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def stage_history_param_key(self, true_level):
        return {
            "flat_level": "flat",
            "room_level": "room",
            "zone_level": "zone",
            "level_id": "level_id",        # if you use this fk in StageHistory
            "checklist_level": "checklist" # if you use this fk in StageHistory
        }.get(true_level)

    def get_true_level(self, project_id):
        try:
            resp = requests.get(self.TRANSFER_RULE_API, params={"project_id": project_id}, timeout=5)
            if resp.status_code == 200 and resp.json():
                tl = resp.json()[0].get("true_level")
                self.dbg("true_level", tl=tl)
                return tl
        except Exception as e:
            self.dbg("true_level_err", err=str(e))
        return None

    def get_location_for_true_level(self, request, true_level):
        """
        Resolve the location key + id from query params for the given true_level.
        """
        # allow explicit override: whichever locator the client passed in
        for qp_key in ("flat_id", "room_id", "zone_id", "level_id", "checklist_id"):
            if qp_key in request.query_params:
                return (qp_key.replace("_id", ""), self.safe_nt(request.query_params.get(qp_key)))

        # else map from true_level
        key = self.stage_history_param_key(true_level)
        if not key:
            return (None, None)
        qp_key = f"{key}_id" if not key.endswith("_id") else key
        return (key, self.safe_nt(request.query_params.get(qp_key)))

    def fetch_stage_history(self, *, project_id, key=None, loc_id=None, is_current=None, ordering=None):
        """
        Query StageHistory directly with flexible ordering.
        """
        try:
            filters = {"project": int(project_id)}
            if key and loc_id:
            ...
            self.dbg("fetch_err", err=str(e))
            return []

    def _crm_note_for_completed_row(self, row: dict) -> str:
        """
        Build NOTE like:
        - "Completed by {completed_name} — Handover not done"
        - "Completed by {completed_name} — Handover done by {who} on {date}"
        """
        completed_name = (row.get("completed_by_name") or "").strip() or (
            f"User {row.get('completed_by')}" if row.get("completed_by") else "Someone"
        )
        crm_name = (row.get("crm_completed_by_name") or "").strip()
        crm_by   = row.get("crm_completed_by")
        crm_hoto = row.get("crm_hoto")
        ...
        if crm_done:
            who = crm_name or (f"User {crm_by}" if crm_by else "Someone")
            if crm_date:
                return f"Completed by {completed_name} — Handover done by {who} on {crm_date}"
            return f"Completed by {completed_name} — Handover done by {who}"
        return f"Completed by {completed_name} — Handover not done"
    ...
    def get(self, request):
        ...
        Query params:
          - project_id (required)
          - exactly ONE of: flat_id | room_id | zone_id | level_id | checklist_id
          - include_history=1 (optional)
        ...
        Response:
          {
            ...
            "meta": {
              "current_id": <int|None>,
              "last_id": <int|None>,
              "last_status": "completed|move_to_next_*|started|...",
              "completed_by": <int|None>,
              "completed_by_name": <str|None>,
              "crm": { "done": bool, "by": <int|None>, "by_name": <str|None>, "date": <str|None>, "hoto": <bool|None> },
              "my_completed_stagehistory_id": <int|None>
            },
            "history": [ ... ]   # only when include_history=1
          }
        """
        ...
        key, loc_id = self.get_location_for_true_level(request, true_level)
        if not key or not loc_id:
            return Response({"detail": "Provide one location: flat_id | room_id | zone_id | level_id | checklist_id"}, status=400)

        include_history = str(request.query_params.get("include_history", "0")).lower() in ("1", "true", "yes")

        current_rows = self.fetch_stage_history(project_id=project_id, key=key, loc_id=loc_id, is_current=True)
        last_rows    = self.fetch_stage_history(project_id=project_id, key=key, loc_id=loc_id,
                                                is_current=None, ordering="-completed_at")

        current = current_rows[0] if current_rows else None
        last    = last_rows[0] if last_rows else None

        # Build note (prefer completed last row; otherwise maybe current == started)
        note = None
        if last and (last.get("status") or "").lower() == "completed":
            note = self._crm_note_for_completed_row(last)
        elif current:
            note = "Current stage in progress" if (current.get("status") or "").lower() == "started" else None

        # Meta (only expose personal stagehistory id to the finisher)
        meta = {
            "current_id": current.get("id") if current else None,
            "last_id":    last.get("id") if last else None,
            "last_status": (last.get("status") if last else None),
        ...
        if last and last.get("completed_by") == request.user.id:
            ...
            meta["my_completed_stagehistory_id"] = None

        payload = {"note": note, "meta": meta}
        if include_history:
            payload["history"] = last_rows  # already a list of dicts we built above

        return Response(payload, status=200)


class StageHistoryCRMUpdateAPIView(APIView):
    permission_classes = [IsAuthenticated]

    def post(self, request):
        ser = StageHistoryCRMUpdateSerializer(data=request.data)
        ser.is_valid(raise_exception=True)
        data = ser.validated_data

        # 1) fetch
        try:
            sh = StageHistory.objects.get(id=data["stagehistory_id"])
        except StageHistory.DoesNotExist:
            return Response({"detail": "StageHistory not found"}, status=404)

        # 2) must be COMPLETED to tag CRM
        if (sh.status or "").lower() != "completed":
            return Response(
                {"detail": "Stage must be completed before CRM tagging.",
                 "current_status": sh.status},
                status=400
            )

        # 3) basic permission: finisher or privileged user
        is_finisher = (sh.completed_by == request.user.id)
        is_staff = bool(getattr(request.user, "is_staff", False))
        is_superuser = bool(getattr(request.user, "is_superuser", False))
        has_perm = bool(getattr(request.user, "has_perm", None) and
                        request.user.has_perm("checklists.change_stagehistory"))

        if not (is_finisher or is_staff or is_superuser or has_perm):
            return Response({"detail": "Not allowed"}, status=403)

        # 4) apply fields
        # crm_hoto: default True if not sent; if sent, accept the value
        sh.crm_hoto = data.get("crm_hoto", True)

        # crm_date: if missing/null, set to today
        crm_date = data.get("crm_date") or timezone.now().date()
        sh.crm_date = crm_date

        # always auto-fill who tagged the CRM
        sh.crm_completed_by = request.user.id
        name = None
        ...
        sh.crm_completed_by_name = name

        sh.save(update_fields=[
            "crm_hoto", "crm_date", "crm_completed_by", "crm_completed_by_name"
        ])

        return Response({
            "detail": "CRM fields updated",
            "stagehistory_id": sh.id,
            "crm": {
                "crm_hoto": sh.crm_hoto,
                "crm_date": sh.crm_date,
                "crm_completed_by": sh.crm_completed_by,
                "crm_completed_by_name": sh.crm_completed_by_name,
            }
        }, status=200)
...
class FlatReportAPIView(APIView):
    ...
    def get(self, request, flat_id):
        ...
        # StageHistory → completion banner block
        sh_rows = self._get_flat_stagehistory(project_id, flat_id)
        data["completion"] = self._compute_completion_banner(sh_rows, token)
        ...
        if true_level == "flat_level":
            ...
            # Non-flat true_level: fall back to flat list of checklists
            data["checklists"] = []
            for checklist in checklists:
                checklist_data = {
                    "id": checklist.id,
    ...
    def _get_flat_details(self, flat_id, token):
        ...
        # --- Resolve names ---
        if flat.get("project"):
            project_resp = requests.get(f"{self.BASE_PROJECT_API}/projects/{flat['project']}/", headers=headers)
    ...
    def _get_stage_name(self, stage_id, token):
        headers = {"Authorization": f"Bearer {token}"} if token else {}
        try:
            resp = requests.get(f"{self.BASE_STAGE_API}/{stage_id}/info/", headers=headers, timeout=5)
            if resp.status_code == 200:
                data = resp.json()
                return data.get("stage_name") or data.get("name") or f"Stage {stage_id}"
        except Exception as e:
        ...
        for checklist in checklists:
            for item in checklist.items.all():
                ...
                if item.status in ["not_started", "pending_for_maker", "pending_for_supervisor"]:
                    ...
                    completed += 1
                elif str(item.status).startswith("rejected"):
                    rejected += 1
        return {
    ...
    # --------------------------
    # NEW: StageHistory helpers
    # --------------------------

    def _get_flat_stagehistory(self, project_id, flat_id):
        """
        Return recent StageHistory rows for this flat (most-recent first).
        """
        qs = (
            StageHistory.objects
            .filter(project=project_id, flat=flat_id)
            .order_by('-completed_at', '-started_at', '-id')
        )
        return list(qs.values(
            'id', 'status', 'is_current', 'completed_at',
            'completed_by', 'completed_by_name',
            'crm_hoto', 'crm_date', 'crm_completed_by', 'crm_completed_by_name',
            'stage', 'phase_id'
        ))

    def _compute_completion_banner(self, sh_rows, token):
        """
        Decide what to show in the header:
        - If any row with status='completed' exists, show the latest one:
          Completed by <name> on <completed_at> + Handover (CRM) status
        - Else, no completion banner.
        """
        if not sh_rows:
            return {"is_completed": False}

        completed_rows = [r for r in sh_rows if (r.get('status') or '').lower() == 'completed']
        if not completed_rows:
            return {"is_completed": False}

        latest = completed_rows[0]

        # Resolve finisher name if missing
        completed_by_name = latest.get('completed_by_name')
        if not completed_by_name and latest.get('completed_by'):
            completed_by_name = self._get_user_name(latest['completed_by'], token)

        return {
            "is_completed": True,
            "stagehistory_id": latest["id"],
            "completed_by_name": completed_by_name,
            "completed_at": latest.get("completed_at"),
            "crm_hoto": bool(latest.get("crm_hoto")),
            "crm_date": latest.get("crm_date"),
            "crm_completed_by_name": latest.get("crm_completed_by_name"),
        }
...
ORG_SERVICE_URL = "http://127.0.0.1:8002/api/organizations/"
PROJECT_SERVICE_URL = "http://127.0.0.1:8003/api/projects/"
CHECKLIST_SERVICE_URL = "http://127.0.0.1:8004/api/checklists/"  

class DashboardAPIView(APIView):
    def get(self, request):
        try:
            # Get all data from other services dynamically
            users_res = requests.get(USER_SERVICE_URL)
            orgs_res = requests.get(ORG_SERVICE_URL)
            projects_res = requests.get(PROJECT_SERVICE_URL)
            checklists_res = requests.get(CHECKLIST_SERVICE_URL)
            ...
            checklists = checklists_res.json() if checklists_res.status_code == 200 else []

            dashboard_data = {
                "inspection_status": {
                    "completed": len([c for c in checklists if c["status"] == "completed"]),
                    "pending": len([c for c in checklists if c["status"] == "not_started"]),
                    "rejected": len([c for c in checklists if c["status"] == "work_in_progress"]),
                },
                "snag_summary": {
                    "raised_by_inspector": len(users),
                    "verified": len(projects),
                    "reopened": len(orgs),
                },
                "handover": {
                    "units_given": len(checklists),
                    "units_rechecked": len([c for c in checklists if c["status"] == "in_progress"]),
                },
            }
